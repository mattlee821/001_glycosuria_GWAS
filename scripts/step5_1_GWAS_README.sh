...
cd ./GWAS/ALSPAC/step5_GWAS

#run test gwas on single CHR to test
snptest_v2.5.2 -data ./GWAS/ALSPAC/step4_filter_SNPs/output/step2_SNP2KEEP_filter/output/data_chr16_sample_filter_subsetted.bgen ./GWAS/ALSPAC/step1_phenofile/pheno_file_include-list-only.txt -o data_chr16.out -log data_chr16.log -frequentist 1 -method expected -pheno e113 -cov_names PC1 PC2 PC3 PC4 PC5 PC6 PC7 PC8 PC9 PC10 -use_raw_phenotypes
##########################	
...
snptest_v2.5.2 -data ./GWAS/ALSPAC/step4_filter_SNPs/output/step2_SNP2KEEP_filter/output/${VAR1} ./GWAS/ALSPAC/step1_phenofile/pheno_file_include-list-only.txt -o ./GWAS/ALSPAC/step5_GWAS/output/${VAR1%.bgen}.out -log ${VAR1%.bgen}.log -frequentist 1 -method expected -pheno e113 -cov_names PC1 PC2 PC3 PC4 PC5 PC6 PC7 PC8 PC9 PC10 -use_raw_phenotypes
##########################	

snptest_v2.5.2 -data ./GWAS/ALSPAC/step4_filter_SNPs/output/step2_SNP2KEEP_filter/data_chr01_sample_filter_subsetted.bgen ./GWAS/ALSPAC/step1_phenofile/pheno_file_include-list-only.txt -o ./GWAS/ALSPAC/step5_GWAS/output/chr1.out -log chr1.log -frequentist 1 -method expected -pheno glycosuria_questionnaire_e113 -cov_names PC1 PC2 PC3 PC4 PC5 PC6 PC7 PC8 PC9 PC10 -use_raw_phenotypes
##########################	

#create .txt file of all names for submission scripts based on files in a directory
ls -1 ./GWAS/ALSPAC/step4_filter_SNPs/output/step2_SNP2KEEP_filter/output > data_file_names.txt

#edit the ouput from this to have a single chr per line

#create multiple qsub scripts based on master_sub
cat data_file_names.txt | while read i; do echo ${i}; awk '{ if (NR == 7) print "VAR1='${i}'"; else print $0}' master_sub > ${i}.sh; done
...
wc -l GWAS_output_raw.txt # 9402819 (extra line for header)
###################################################################################################

##############
######check allele and name column innext step
##############
#EA=A NEA=G

head GWAS_header.txt
grep rs13337037 data_chr16_sample_filter_subsetted.out 

###########################
# tidy final GWAS file up for making plots
cd ./GWAS/ALSPAC/step5_GWAS/output
