...
cd ./GWAS/ALSPAC/step4_filter_SNPs
...
datadir = "./GWAS/ALSPAC/step3_create_SNP_stats/output/"
files = list.files(datadir)
w = grep("_snpstats.txt", files)

#### 
dataOUTdir = "./GWAS/ALSPAC/step4_filter_SNPs/output/filter/"
...
cd ./GWAS/ALSPAC/step4_filter_SNPs/output/step2_SNP2KEEP_filter

dir1=./GWAS/ALSPAC/step2_subsample_genetic_data/output/
dir2=./GWAS/ALSPAC/step4_filter_SNPs/output/filter/SNP2Keep/
...
ls -1 ./GWAS/ALSPAC/step2_subsample_genetic_data/output > subset_filenames.txt

#create multiple qsub scripts based on master_sub
cat subset_filenames.txt | while read i; do echo ${i}; awk '{ if (NR == 14) print "VAR1='${i}'"; else print $0}'  master_sub > ${i}.sh; done
